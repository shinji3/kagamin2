...
namespace Kagamin2
{
    ...
    class DateTimeHandler : IHTTPRequestHandler
    {
        ...
        public void HandleRequest(HTTPServerRequest request, HTTPServerResponse response)
        {
                ...
                bool _deny = false;
                bool LogACC = true;
                string _resolve = request.RemoteAddr;
                ...
                {
#if !DEBUG
                if (_resolve.StartsWith("192.168.") || _resolve.StartsWith("10.") || _resolve.StartsWith("127."))
                    _deny = false;
                else
                {
#endif
                    _resolve = Dns.GetHostEntry(request.RemoteAddr).HostName.ToLower();
                    foreach (string _deny_host in Front.Acl.HpDenyRemoteHost)
                    {
                        ...
                        try
                        {
                            ...
                            Regex re = new Regex(_deny_host, RegexOptions.IgnoreCase);
                            if (re.IsMatch(_resolve))
                                _deny = true;
                        }
                        catch { }
                        if (_deny_host == request.RemoteAddr.ToLower())
                            _deny = true;
                        if (_resolve.StartsWith(_deny_host.ToLower()) ||
                            _resolve.EndsWith(_deny_host.ToLower()))
                            _deny = true;
                    }
                    foreach (string _deny_ip in Front.DenyLogin)
                    {
                        if (_deny_ip == "")
                            continue;
                        if (_deny_ip == request.RemoteAddr)
                            _deny = true;
                    }
#if !DEBUG  
            }
                ...
                #region Basic認証
                if (Front.Hp.AuthEnable)
                {
                    if (request.Get("Authorization") == string.Empty ? true : false)
                    {
                        #region HP認証失敗 401 Authenticate
                        response.ContentType = "text/html; charset=UTF-8";
                        response.Add("WWW-Authenticate", "Basic realm=\"Secret\"");
                        response.Status = HTTPServerResponse.HTTPStatus.HTTP_UNAUTHORIZED;
                        resp = 401;
                        try
                        {
                            using (Stream ostr = response.Send())
                            using (TextWriter tw = new StreamWriter(ostr))
                            {
                                tw.WriteLine("<html><head><title>401 Authorization Required</title></head>\r\n" +
                                    "<body><h1>401 Authorization Required</h1></body></html>");
                            }
                        }
                        catch (Exception e)
                        {
                            Front.AddLogDebug("HttpHybrid", "Authenticate内部エラー:" + e.Message + "/Trace:" + e.StackTrace);
                        }
                        return;
                        #endregion
                    }
                    else
                    {
                        try
                        {
                            string _auth = request.Get("Authorization");
                            _auth = Encoding.ASCII.GetString(Convert.FromBase64String(_auth.Substring(6)));
                            if (_auth.Split(':')[0] == Front.Hp.AuthUser &&
                                _auth.Split(':')[1] == Front.Hp.AuthPass)
                            {
                                /*
                                using (Stream ostr = response.Send())
                                using (TextWriter tw = new StreamWriter(ostr))
                ...
                if (request.URI == "/" || request.URI.StartsWith("/index.html"))
                {
                    ...
                    try
                    {
                        ...
                        using (TextWriter tw = new StreamWriter(ostr))
                        {
                            foreach (string s in TemplateFrame)
                                tw.WriteLine(s.Replace("<GlobalIP>", Front.GlobalIP));
                        }
                    }
                    catch (Exception e)
                    {
                ...
                else if (request.URI.StartsWith("/main.html"))
                {
                    ...
                    try
                    {
                        ...
                        using (TextWriter tw = new StreamWriter(ostr))
                        {
                            ...
                            {
                                tw.WriteLine(TemplateMain[i].Replace("<VERSION>", Front.AppName));
                                i++;
                            }
                            ...
                            foreach (Kagami k in Front.KagamiList)
                            {
                                ...
                                tw.WriteLine(
                                    ...
                                    .Replace("<MAX_CON>", k.Status.ConnectionMax.ToString() + "人")
                                    .Replace("<TITLE>", k.Status.ASFTitle)
                                    .Replace("<AUTHOR>", k.Status.ASFAuthor)
                                    .Replace("<COPYRIGHT>", k.Status.ASFCopyRight)
                                    .Replace("<DESCRIPTION>", k.Status.ASFDescription)
                                    .Replace("<RATING>", k.Status.ASFRating)
                ...
                else if (request.URI.StartsWith("/all.html") && Front.Hp.allh)
                {
                    #region /all.htmlへのリクエスト
                    LogACC = false;
                    response.ContentType = "text/plane; charset=UTF-8";
                    response.StatusAndReason = HTTPServerResponse.HTTPStatus.HTTP_OK;
                    try
                    {
                        ...
                        using (TextWriter tw = new StreamWriter(ostr))
                        {
                            ...
                            foreach (Kagami k in Front.KagamiList)
                            {
                                if (k.Status.Type == 0)
                                    continue;
                                num++;
                                _port += k.Status.MyPort.ToString() + ",";
                            }

                            tw.WriteLine("PORT_NUM:" + num);
                            tw.WriteLine("PORT:" + _port + "\r\n");
                            foreach (Kagami k in Front.KagamiList)
                            {
                                if (k.Status.Type == 0)
                                    continue;
                                ...
                                //tw.WriteLine("<br>");
                                tw.WriteLine("PORT:" + k.Status.MyPort);
                                tw.WriteLine("TITLE:" + k.Status.Comment);
                                tw.WriteLine("MAX_CONN:" + k.Status.Connection.ToString());
                                tw.WriteLine("CONN:" + k.Status.Client.Count);
                                tw.WriteLine("COUNT:" + count.ToString());
                                tw.WriteLine("MY_URL:" + Front.Hp.IpHTTP + ":" + k.Status.MyPort.ToString());
                                tw.WriteLine("SRC_URL:" + (k.Status.UrlVisible ? k.Status.ImportURL : "設定が非表示になっています"));
                                tw.WriteLine("MAXCONN:" + k.Status.Connection.ToString() + ((k.Status.Reserve != 0) ? "+" + k.Status.Reserve.ToString() : ""));
                                tw.WriteLine("COMMENT:" + k.Status.Comment);
                                tw.WriteLine("PAUSE:" + (Front.Pause ? "START" : "STOP"));
                                tw.WriteLine("BANDWIDTH:" + (Front.BndWth.EnableBandWidth ?
                                        (k.Status.LimitUPSpeed >= 1000 ?
                                            (k.Status.LimitUPSpeed / 1000).ToString() + "Mbps" :
                                            k.Status.LimitUPSpeed.ToString() + "Kbps"
                                        ) : "-"));
                                tw.WriteLine("BITRATE:" + k.Status.MaxDLSpeed.ToString() + "Kbps");
                                tw.WriteLine("BUSYCOUNT:" + k.Status.BusyCounter.ToString());
                                tw.WriteLine("IMPORT_ERROR:" + k.Status.ImportError.ToString());
                                tw.WriteLine("EXPORT_ERROR:" + k.Status.ExportError.ToString());
                                tw.WriteLine("TIME:" + k.Status.ImportTimeString);
                                tw.WriteLine("LIVE_URL:" + k.Status.Url);
                                tw.WriteLine("ERROR:" + k.Status.ImportErrorContext);
                                tw.WriteLine("MAX_CON:" + k.Status.ConnectionMax.ToString());
                                tw.WriteLine("RETRY_COUNT:" + (k.Status.Type == 2 ? "" : k.Status.RetryCounter.ToString() + "/" + Front.Retry.OutRetryTime.ToString()));
                                tw.WriteLine("SET:" + str_tmp);
                                count++;
                            }
                    ...
                    {
                        Front.AddLogDebug("HttpHybrid", "all.html内部エラー:" + e.Message + "/Trace:" + e.StackTrace);
                    }
                    #endregion
                }
                else if (request.URI.StartsWith("/port.html?") && Front.Hp.porth)
                {
                    LogACC = false;
                    #region /port.htmlへのリクエスト
                    response.ContentType = "text/plane; charset=UTF-8";
                    response.StatusAndReason = HTTPServerResponse.HTTPStatus.HTTP_OK;
                    try
                    {
                        ...
                        using (TextWriter tw = new StreamWriter(ostr))
                        {
                            string prm = request.URI.Replace("/port.html?", "");
                            ...
                            if (dic.ContainsKey("port"))
                            {
                                ...
                                if (k != null)
                                {
                                    tw.WriteLine("PORT:" + port.ToString());
                                    tw.WriteLine("RESERVE:" + k.Status.Reserve.ToString());
                                    //tw.WriteLine("ADMIN_PASS:");
                                    tw.WriteLine("ENABLE_PUSH:", Front.Opt.EnablePush ? "" : "disabled");
                                    tw.WriteLine("PUSH_CHECK:", k.Status.PushOnly ? "checked=\"checked\"" : "");
                                    tw.WriteLine("PULL_CHECK:", !k.Status.PushOnly ? "checked=\"checked\"" : "");
                                    tw.WriteLine("ENABLE_TRANS:", Front.Opt.TransKagamin ? "" : "disabled");
                                    tw.WriteLine("PUSH_ONLY:", k.Status.PushOnly ? "disabled" : "");
                                    tw.WriteLine("LIVEOUT_URL:", Front.Opt.OutUrl);
                    ...
                    {
                        Front.AddLogDebug("HttpHybrid", "port.html内部エラー:" + e.Message + "/Trace:" + e.StackTrace);
                    }
                    #endregion
                }
                                    ...
                                    if (dic.ContainsKey("admin"))
                                    {
                                        ...
                                        foreach (string s in TemplateConn)
                                        {
                                            tw.WriteLine(s
                                                ...
                                                .Replace("<PUSH_CHECK>", k.Status.PushOnly ? "checked=\"checked\"" : "")
                                                .Replace("<PULL_CHECK>", !k.Status.PushOnly ? "checked=checked" : "")
                                                .Replace("<ENABLE_PUSH>", Front.Opt.EnablePush ? "" : "disabled")
                                                //.Replace("<ENABLE_SQL>", Front.Opt.MySQLEnable ? "" : "disabled")
                                                //.Replace("<ENABLE_VHOST>", Front.Sock.VirtualHost ? "" : "disabled")
                                                .Replace("<ENABLE_TRANS>", Front.Opt.TransKagamin ? "" : "disabled")
                                                .Replace("<LIVEOUT_URL>", Front.Opt.OutUrlstr)
                                                .Replace("<PUSH_ONLY>", k.Status.PushOnly ? "disabled" : "")
                                                .Replace("<IP_ADDR>", request.RemoteAddr)
                                            );
                                        }
                                    }
                                    else
           ...
           dic["comment"].IndexOf("+AD4-") != -1)      // UTF-7 XSS
                                    ...
                                    //Close
                                    if (Front.Acl.ClosedPassward != "" && Front.Acl.ClosedPassward != dic["password"])
                                    {
                                        if (Front.Acl.ClosedComment != "")
                                            comment = Front.Acl.ClosedComment;
                                        else
                                        {
                                            comment = "Do not use it without permission.";
                                            Front.FailureLogin.Add(request.RemoteAddr);

                                            if (Front.Acl.FailureCount > 0)
                                            {
                                                int _deny_count = 0;
                                                foreach (string _deny_ip in Front.FailureLogin)
                                                {
                                                    if (_deny_ip == request.RemoteAddr)
                                                        _deny_count++;
                                                }
                                                //指定回数以上ログインに失敗している場合拒否リストに追加（Option.cs:132）
                                                if (_deny_count >= Front.Acl.FailureCount)
                                                {
                                                    Front.DenyLogin.Add(request.RemoteAddr);
                                                }
                                            }
                                        }
                                        break;
                                    ...
                                    {
                                        if (!Front.Opt.OutUrlContain(dic["url"]) ||
                                            dic["url"].IndexOf("<") != -1 ||
                                            dic["url"].IndexOf(">") != -1 ||
                                            dic["url"].IndexOf("\"") != -1 ||
                                    ...
                                    //配信種別で振り分け
                                    if (dic["mode"] == "pull" && !k.Status.PushOnly)
                                    {
                                        // pull配信
                                        //インポート先ホスト名取得
                                        ...
                                        foreach (string sss in Front.Acl.DenyImportURL)
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                try
                                                {
                                                    _sss = Dns.GetHostAddresses(sss)[0].ToString();
                                                    _imp_ip = Dns.GetHostAddresses(imp)[0].AddressFamily.ToString();
                                                }
                                                catch
                                                {
                                                }
                                        ...
                                        if (Front.Opt.WebKick)
                                        {
                                            k.Status.Kick.Clear();
                                            Event.EventUpdateKick(k, null, 2);   // GUIのListViewからListViewItem全削除
                                            k.Status.Gui.KickItem.Clear();       // ReserveItemのListViewItem全削除
                                        }
                                        // リザーブ再設定
                                        Regex r = new Regex(",");
                                        ...
                                        if (dic.ContainsKey("kick"))
                                        {

                                            if (dic["kick"].Length > 0 && Front.Opt.WebKick)
                                            {
                                                kick_list = r.Split(dic["kick"]);

                                                // IPに変換できないホスト名なら登録あきらめ
                                                for (int cnt = 0; cnt < kick_list.Length; cnt++)
                                                {
                                                    try
                                                    {
                                                        System.Net.IPAddress hostadd = System.Net.Dns.GetHostAddresses(kick_list[cnt])[0];

                                                        k.Status.AddKick(hostadd.ToString(), -1);
                                                    }
                                                    catch { }
                                                }
                                        ...
                                        if (Front.Opt.TransKagamin == false)
                                        {
                                            k.Status.TransWeb = false;
                                        }
                                        else
                                        {
                                            if (dic.ContainsKey("trans"))
                                            {
                                                k.Status.TransWeb = (dic["trans"] == "on" ? true : false);
                                            }
                                            else
                                            {
                                                k.Status.TransWeb = true;
                                            }
                                        }
                                        ...
                                        if (dic.ContainsKey("url"))
                                            ...
                                            k.Status.Url = "";
                                        if (Front.Opt.AuthWebSet && dic.ContainsKey("authid") && dic.ContainsKey("authpass"))
                                        {
                                            if (dic["authid"].Length > 0 && dic["authpass"].Length > 0)
                                            {
                                                k.Status.AuthID = dic["authid"];
                                                k.Status.AuthPass = dic["authpass"];
                                                k.Status.ExportAuth = true;
                                            }
                                            else
                                            {
                                                k.Status.AuthID = "";
                                                k.Status.AuthPass = "";
                                                k.Status.ExportAuth = false;
                                            }
                                        }
                                        else
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        if (dic.ContainsKey("kick"))
                                        {
                                            ...
                                            {
                                                Regex r = new Regex(",");
                                                string[] kick_list = r.Split(dic["kick"]);
                                        ...
                                        //k.Status.SQLOn = (dic["sql"] == "on" ? true : false);
                                        //k.Status.VirtualHost = (dic["vhost"] == "on" ? true : false);
                    ...
                    try
                    {
                        ...
                        using (TextWriter tw = new StreamWriter(ostr))
                        {
                            ...
                            else if (dic.ContainsKey("password"))
                            {
                                ...
                                {
                                    if ((k.Status.Password == dic["password"] && dic["password"] != "") ||
                        ...
                        using (TextWriter tw = new StreamWriter(ostr))
                        {
                            ...
                            while (true)
                            {
                                    ...
                                    !dic.ContainsKey("reserve"))
                                {
                                    ...
                                    //Kickリストをカンマ区切り文字列にする
                                    string kick_list = "";
                                    foreach (ListViewItem r in k.Status.Gui.KickItem)
                                        kick_list += r.Text + ",";
                                    if (kick_list.Length > 0)
                                        kick_list = kick_list.Remove(kick_list.Length - 1);
                                    ...
                                    foreach (string s in TemplateSet)
                                    {
                                        tw.WriteLine(s
                                            ...
                                            //.Replace("<SQL_ON>", (k.Status.SQLOn == true ? "checked=\"checked\"" : ""))
                                            //.Replace("<SQL_OFF>", (k.Status.SQLOn == false ? "checked=\"checked\"" : ""))
                                            //.Replace("<VHOST_ON>", (k.Status.VirtualHost == true ? "checked=\"checked\"" : ""))
                                            //.Replace("<VHOST_OFF>", (k.Status.VirtualHost == false ? "checked=\"checked\"" : ""))
                                            .Replace("<RESERVE_LIST>", reserve_list)
                                            .Replace("<KICK_LIST>", kick_list)
                                            .Replace("<RESERVE_NUM>", k.Status.Reserve.ToString())
                                            .Replace("<LIVE_URL>", k.Status.Url)
                                            .Replace("<LIVEOUT_URL>", Front.Opt.OutUrlstr)
                                            .Replace("<TRANS_ON>", (k.Status.TransWeb == true ? "checked=\"checked\"" : ""))
                                            .Replace("<TRANS_OFF>", (k.Status.TransWeb == false ? "checked=\"checked\"" : ""))
                                            .Replace("<AUTH_ID>", (Front.Opt.AuthWebSet == true ? k.Status.AuthID : ""))
                                            .Replace("<AUTH_PASS>", (Front.Opt.AuthWebSet == true ? k.Status.AuthPass : ""))
                                        );
                                    }
                                    #endregion
                                }
                                    ...
                                    dic.ContainsKey("radio"))
                                {
                                    ...
                                    if (dic.ContainsKey("kick"))
                                    {
                                        ...
                                        try
                                        {
                                            for (int r_cnt = 0; r_cnt < kick_list.Length; r_cnt++)
                                            {
                                                if (kick_list[r_cnt] != "")
                                                {
                                                    System.Net.IPAddress hostadd = System.Net.Dns.GetHostAddresses(kick_list[r_cnt])[0];
                                                }
                                            }
                                        }
                                        catch
                                    ...
                                    k.Status.UrlVisible = (dic["radio"] == "on" ? true : false);
                                    /*
                                    if (dic.ContainsKey("vhost"))
                                        k.Status.VirtualHost = (dic["vhost"] == "on" ? true : false);
                                    else
                                        k.Status.VirtualHost = false;

                                    if (dic.ContainsKey("sql"))
                                        k.Status.SQLOn = (dic["sql"] == "on" ? true : false);
                                    else
                                        k.Status.SQLOn = false;
                                    */
                                    if (dic.ContainsKey("url"))
                                        k.Status.Url = dic["url"];
                                    else
                                    ...
                                    if (Front.Opt.WebKick)
                                    {
                                        ...
                                        k.Status.Gui.KickItem.Clear();       // kickItemのListViewItem全削除
                                    }
                                    // 受け取ったリスト数、もしくは最大リザーブ登録数の小さい方の値でループ
                                    for (int r_cnt = 0; r_cnt < reserve_list.Length && r_cnt < k.Status.Reserve; r_cnt++)
                                    {
                                    ...
                                    if (dic.ContainsKey("kick"))
                                    {
                                        if (Front.Opt.WebKick && dic.ContainsKey("kick"))
                                        {
                                            for (int r_cnt = 0; r_cnt < kick_list.Length; r_cnt++)
                                            {
                                                try
                                                {
                                                    ...
                                                    {
                                                        System.Net.IPAddress hostadd = System.Net.Dns.GetHostAddresses(kick_list[r_cnt])[0];
                                                        k.Status.AddKick(hostadd.ToString(), -1, true);
                                                    }
                        ...
                        using (TextWriter tw = new StreamWriter(ostr))
                        {
                            ...
                            while (true)
                            {
                                ...
                                if (dic["password"] == "" || dic["password"] != Front.Opt.AdminPass)
                                {
                                    ...
                                    //ログイン失敗の場合リストにIPを追加
                                    Front.FailureLogin.Add(request.RemoteAddr.ToString());
                                    ...
                                    if (Front.Acl.FailureCount > 0)
                                    {
                                        ...
                                        foreach (string _deny_ip in Front.FailureLogin)
                                        {
                                            if (_deny_ip == request.RemoteAddr.ToString())
                                                _deny_count++;
                                        }
                                        //指定回数以上ログインに失敗している場合拒否リストに追加（Option.cs:132）
                                        if (_deny_count >= Front.Acl.FailureCount)
                                        {
                                            Front.DenyLogin.Add(request.RemoteAddr.ToString());
                                            foreach (string s in TemplateAdminOk)
                                                tw.WriteLine(s
                                                    .Replace("<Status>", "指定回数失敗しました。接続を拒否します。")
                                                    .Replace("<ADMIN_PASS>", "")
                                                    );
                                        }
                                    }
                                ...
                                if (!dic.ContainsKey("mode"))
                                {
                                    ...
                                    try
                                    {
                                        _band = Front.BandStopTypeString[Front.BndWth.BandStopMode];
                                    }
                                    catch
                                    {
                                    }
                                    ...
                                    while (TemplateAdminMain.Count > i && TemplateAdminMain[i] != "<TEMPLATE>")
                                    {
                                        tw.WriteLine(TemplateAdminMain[i]
                                            ...
                                            .Replace("<BAND_NUM>", Front.BndWth.BandStopValue.ToString())
                                        ...
                                        */
                                        ...
                                        else
                                        {
                                            ...
                                            tw.WriteLine(
                                                ...
                                                .Replace("<BAND>", Front.BndWth.EnableBandWidth ? "開始中" : "停止中")
                                                .Replace("<BAND_TYPE>", _band)
                                                .Replace("<BAND_NUM>", k.Status.LimitUPSpeed.ToString())
                                                .Replace("<ADMIN_PASS>", Front.Opt.AdminPass)       // 管理モード追加タグ
                                                .Replace("<COUNT>", count.ToString())
                                                .Replace("<MY_URL>", Front.Hp.IpHTTP + ":" + k.Status.MyPort.ToString())
                                                .Replace("<PORT>", k.Status.MyPort.ToString())
                                                ...
                                                .Replace("<BUSYCOUNT>", k.Status.BusyCounter.ToString())
                                                .Replace("<TIME>", k.Status.ImportTimeString)
                                                .Replace("<AUTH_ID>", k.Status.ExportAuth ? k.Status.AuthID : "無効")
                                                .Replace("<AUTH_PASS>", k.Status.ExportAuth ? k.Status.AuthPass : "無効")
                                            );
                                        }
                                        count++;
                                    }
                                ...
                                else
                                {
                                    ...
                                    switch (dic["mode"])
                                    {
                                        ...
                                        case "bandchg":
                                            #region 帯域制限方式の切り替え
                                            if (Front.BandStopTypeString.Length - 1 == Front.BndWth.BandStopMode)
                                            {
                                                Front.BndWth.BandStopMode = 0;
                                            }
                                            else
                                            {
                                                Front.BndWth.BandStopMode++;
                                            }
                                            comment = "帯域制限方式を" + Front.BandStopTypeString[Front.BndWth.BandStopMode] +
                                                "<br>へ切り替えました";
                                            Event.EventUpdateKagami();
                                            #endregion
                                            break;
                                        case "bandup":
                                            #region 帯域制限量アップ
                                            if (!dic.ContainsKey("port") || Front.BndWth.BandStopMode != 2)
                                            {
                                                Front.BndWth.BandStopUnit = 2;
                                                Front.BndWth.BandStopValue += 1000;
                                                comment = "帯域全体値を" + Front.BndWth.BandStopValue + "Kbpsへ変更しました";
                                                Event.EventUpdateKagami();
                                                break;
                                            }
                                            try
                                            ...
                                            _k.Status.LimitUPSpeed += 1000;
                                            comment = "ポート" + _port + "の帯域制限値を" + _k.Status.LimitUPSpeed + "Kbpsへ変更しました";
                                            Event.EventUpdateKagami();
                                            break;
                                            #endregion
                                        case "banddown":
            ...
            finally
            {
                ...
                try
                {
                    if (Front.Log.HpLogFile.Length != 0 && LogACC == true)
                    {
                        lock (Front.Log.HpLogFile)
                        {
                            ...
                            string logFile = Front.FilePathTrans(Front.Log.HpLogFile);
                            ...
                            string str = request.RemoteAddr + " - " + _resolve + " - - [" + _dtNow.ToString("yyyy/MM/dd hh:mm:ss") + "] \"" + request.Method + " "
